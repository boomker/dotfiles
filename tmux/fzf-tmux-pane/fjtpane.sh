...
get_fzf_options() {
    local fzf_options
    local tmux_cmd="tmux capture-pane -pJS -20 -t {1}"
    local fzf_default_options='-w75% -h75% -m -e -0 --cycle --border --ansi --preview-window=up:70%:wrap --preview "${tmux_cmd}"'
...
items=$(tmux list-panes -s -F '#I.#P - #W - #T #{pane_current_path} #{pane_current_command}' | gsed "s|$HOME|~|g" | column -t)
